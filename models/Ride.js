...
const pointSchema = new mongoose.Schema({
  ...
  coordinates:{type:[Number],required:true,validate:{validator:v=>v.length===2&&Number.isFinite(v[0])&&Number.isFinite(v[1])&&v[0]>=-180&&v[0]<=180&&v[1]>=-90&&v[1]<=90,message:'Invalid coordinates'}},
...
const rideSchema=new mongoose.Schema({
  ...
  pickup:{type:pointSchema,required:true},dropoff:{type:pointSchema,required:true},
  estimatedDistanceKm:{type:Number,required:true,min:0},estimatedDurationMin:{type:Number,required:true,min:0},estimatedFare:{type:Number,required:true,min:0},finalFare:{type:Number,min:0,default:null},
