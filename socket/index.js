  ...
  io.on('connection', socket => {
    ...
    socket.on('ride:join', async ({ rideId } = {}, ack) => {
      try {
        ...
        const ride = await Ride.findById(rideId).select('customer captain');
        if (!ride) throw new Error('Ride not found');
        let allowed = user.role === 'admin' || ride.customer.equals(user._id);
        if (user.role === 'captain') {
          const captain = await Captain.findOne({ user: user._id }).select('_id');
          allowed = !!captain && ride.captain?.equals(captain._id);
        }
        if (!allowed) throw new Error('Forbidden');
        await socket.join(roomForRide(rideId));
...
module.exports = { initializeSocket, roomForUser, roomForRide };
