  ...
  try {
    ...
    if (name !== undefined) {
      ...
      if (n.length < 2 || n.length > 80) {
        const e = new Error('Name must be between 2 and 80 characters');
        e.statusCode = 400;
        throw e;
      }
      user.name = n;
    ...
    if (phone !== undefined) {
      ...
      const exists = await User.findOne({ phone: p, _id: { $ne: user._id } });
      if (exists) {
        const e = new Error('Phone is already registered');
        e.statusCode = 409;
        throw e;
      }
      user.phone = p;
    ...
    if (avatarUrl !== undefined) {
      user.profile.avatarUrl = String(avatarUrl).trim().slice(0, 500);
    }
