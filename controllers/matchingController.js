...
async function claim(req,res,next){try{const captain=await Captain.findOne({user:req.user._id,status:'active',availability:'online'});if(!captain)throw fail('Captain is not available','CAPTAIN_NOT_AVAILABLE',409);if(!mongoose.isValidObjectId(req.params.id))throw fail('Ride not found','RIDE_NOT_FOUND',404);const ride=await claimNextCaptain(req.params.id,captain._id);if(!ride)throw fail('Ride is no longer available','RIDE_UNAVAILABLE',409);return success(res,ride);}catch(e){next(e);}}
