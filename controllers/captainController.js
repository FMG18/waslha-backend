...
async function updateAvailability(req, res, next) {
  try {
    ...
    const captain = await Captain.findOneAndUpdate({ user: req.user._id, status: 'active' }, { availability }, { new: true });
    if (!captain) { const e = new Error('Active captain profile not found'); e.statusCode = 404; e.code = 'CAPTAIN_NOT_FOUND'; throw e; }
...
async function updateLocation(req, res, next) {
  try {
    ...
    const captain = await Captain.findOneAndUpdate({ user: req.user._id }, { currentLocation: { lat, lng, updatedAt: new Date() } }, { new: true });
