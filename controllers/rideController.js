...
function bad(message, code = 'VALIDATION_ERROR', statusCode = 400) { const error = new Error(message); error.code = code; error.statusCode = statusCode; return error; }
function readPoint(value, name) { const lat = Number(value?.lat); const lng = Number(value?.lng); if (!Number.isFinite(lat) || !Number.isFinite(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) throw bad(`Invalid ${name} coordinates`, 'INVALID_COORDINATES'); return { lat, lng }; }
...
async function getById(req, res, next) {
  try {
    ...
    const isAdmin = req.user.role === 'admin'; const isCustomer = ride.customer._id.equals(req.user._id); const isCaptain = req.user.role === 'captain' && ride.captain?.user?._id?.equals(req.user._id);
...
async function captainAction(req, res, next) {
  try {
    ...
    const nextStatus = { arriving: 'captain_arriving', arrived: 'captain_arrived', start: 'trip_started', complete: 'trip_completed' }[req.params.action];
...
async function cancel(req, res, next) {
  try {
    if (!mongoose.isValidObjectId(req.params.id)) throw bad('Ride not found', 'RIDE_NOT_FOUND', 404);
    const ride = await Ride.findById(req.params.id); if (!ride) throw bad('Ride not found', 'RIDE_NOT_FOUND', 404);
    const isCustomer = req.user.role === 'customer' && ride.customer.equals(req.user._id);
    let isCaptain = false; if (req.user.role === 'captain') { const captain = await Captain.findOne({ user: req.user._id }); isCaptain = !!captain && ride.captain?.equals(captain._id); }
    if (req.user.role !== 'admin' && !isCustomer && !isCaptain) throw bad('You do not have access to this ride', 'FORBIDDEN', 403);
    const updated = await changeStatus(ride, 'cancelled'); updated.cancellation.cancelledBy = req.user.role; updated.cancellation.reason = String(req.body?.reason || '').trim().slice(0, 300); updated.cancellation.cancelledAt = new Date(); await updated.save();
